...
function displayCartItems(id, title, price, imageUrl, quantity)
{
    ...
    async function changeQuantity(quantity)
    {
        try{    
            ...
            const updateCart=await axios.post(`http://3.87.122.85/changeQuantity`,{id, quantity},{headers});
...
document.addEventListener('DOMContentLoaded',async ()=>{
    const cart=await axios.get('http://3.87.122.85/getCartItems',{headers});
    if(cart.status==200)
    {
        ...
        if(cartItems.length)
        {
            ...
            createOrder.addEventListener('click',async (e)=>{
                e.preventDefault();
                const order=await axios.get('http://3.87.122.85/createOrder',{headers});
                if(order.status === 200)
                {
